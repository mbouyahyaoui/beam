...
/**
 * An implementation of the Beam {@link StateInternals}. This implementation simply keeps
 * elements in memory.
 * This state is periodically checkpointed by Flink, for fault-tolerance.
 * <p>
 * TODO: State should be rewritten to redirect to Flink per-key state so that coders and
 * combiners don't need
...
public class FlinkStateInternals<K> implements StateInternals<K> {
  ...
  protected final StateTable<K> inMemoryState = new StateTable<K>() {
    @Override
    protected StateTag.StateBinder binderForNamespace(final StateNamespace namespace, final
    StateContext<?> c) {
      return new StateTag.StateBinder<K>() {

        @Override
        public <T> ValueState<T> bindValue(StateTag<? super K, ValueState<T>> address, Coder<T>
            coder) {
          return new FlinkInMemoryValue<>(encodeKey(namespace, address), coder);
        }

        @Override
        public <T> BagState<T> bindBag(StateTag<? super K, BagState<T>> address, Coder<T>
            elemCoder) {
