 ...
 */
public class FlinkParDoBoundMultiWrapper<InputT, OutputT> extends
    FlinkAbstractParDoWrapper<InputT, OutputT,
        RawUnionValue> {

  private final TupleTag<?> mainTag;
  private final Map<TupleTag<?>, Integer> outputLabels;

  public FlinkParDoBoundMultiWrapper(PipelineOptions options, WindowingStrategy<?, ?>
      windowingStrategy, OldDoFn<InputT, OutputT> doFn, TupleTag<?> mainTag, Map<TupleTag<?>,
      Integer>
                                         tagsToLabels) {
  ...
  @Override
  public void outputWithTimestampHelper(WindowedValue<InputT> inElement, OutputT output, Instant
      timestamp, Collector<WindowedValue<RawUnionValue>> collector) {
  ...
  @Override
  public <T> void sideOutputWithTimestampHelper(WindowedValue<InputT> inElement, T output, Instant
      timestamp, Collector<WindowedValue<RawUnionValue>> collector, TupleTag<T> tag) {
    checkTimestamp(inElement, timestamp);
  ...
  public WindowingInternals<InputT, OutputT> windowingInternalsHelper(
    ...
    throw new RuntimeException("FlinkParDoBoundMultiWrapper is just an internal operator serving "
        + "as "
